...
def build_estimator():
    ...
    config = tf.estimator.RunConfig(
        model_dir="../../data/checkpoints/ckpt",
        tf_random_seed=2019,
...
def train_eval_save():
    ...
    train_spec = tf.estimator.TrainSpec(
        input_fn=lambda: input_fn(
            path='../../data/tfrecords/train/train.tfrecords',
            parallel_num=32,
            epoch_num=11,
            batch_size=32),
        max_steps=1600
    ...
    eval_spec = tf.estimator.EvalSpec(
        input_fn=lambda: input_fn(
            path='../../data/tfrecords/evaluation/evaluation.tfrecords',
            parallel_num=32,
            epoch_num=1,
            batch_size=32),
        steps=15,     # 验证集评估多少批数据
        start_delay_secs=1,    # 在多少秒后 start_delay_secs=1,    # 在多少秒后
